...
3. 成功時播放音效並發送 Discord / Telegram 通知（`send_discord_notification` / `send_telegram_notification`，`src/nodriver_common.py`）
...
### TicketPlus 訂單失敗彈窗 — `nodriver_ticketplus_accept_order_fail()`（`platforms/ticketplus.py`）
